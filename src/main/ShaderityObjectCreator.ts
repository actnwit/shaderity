...
export default class ShaderityObjectCreator {
	...
	// only define types
	public addStructDefinition(structName: string, variableObjects: ShaderStructVariableObject[]) {
		const isDuplicate =
	...
	public updateStructDefinition(structName: string, variableObjects: ShaderStructVariableObject[]) {
		const matchedIndex =
		...
		this.__structDefinitions[matchedIndex].variableObjects = variableObjects;
	}
	...
	private __createStructDefinitionShaderCode(): string {
		...
		for (const structDefinition of this.__structDefinitions) {
			...
			for (let i = 0; i < structDefinition.variableObjects.length; i++) {
				const variable = structDefinition.variableObjects[i];
				...
				shaderCode += `${variable.type} ${variable.variableName};\n`;
			}
